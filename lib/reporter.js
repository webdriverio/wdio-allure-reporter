...
class AllureReporter extends events.EventEmitter {
    constructor (baseReporter, config, options = {}) {
        ...
        this.on('runner:customCommand', ::this.customCommand)
    ...
    customCommand ({event, cid, customCommand}) {
        ...
        allure.startStep(customCommand.title)

        if (!isEmpty(customCommand.body)) {
            allure.addAttachment(customCommand.bodyLabel, customCommand.body)
        }

        allure.endStep(customCommand.title)
    }
    ...
    static createStep (title, body, bodyLabel = 'attachment') {
        var customCommand = {
            title: title,
            body: body,
            bodyLabel: bodyLabel
        }
        AllureReporter.tellReporter('runner:customCommand', { customCommand })
    }
