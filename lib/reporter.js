    ...
    cucumberSuiteStart (suite) {
        ...
        if (!suite.parent) {
            ...
            if (suite.title) {
                ...
                const browserName = suite.runner[suite.cid].browserName || suite.cid
                const version = suite.runner[suite.cid].version || ''
                test.addParameter('argument', 'browser', `${browserName}-${version}`)
